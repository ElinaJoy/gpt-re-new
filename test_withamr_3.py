...
# 加了优化机制，会有报错但是可以顺利跑完
# 结果：Epoch 3/3, Train Loss: 1.0205, Train Accuracy: 0.6548, Validation Accuracy: 0.6066   2025/07/20
# ---------- 初始化 ----------
...
class RelationDataset(Dataset):
    ...
    def __init__(self, data, max_len=16):
        ...
        # 预计算所有AMR图
        self.amr_graph_cache = self.precompute_amr_graphs()

    def __len__(self):
        return len(self.data)
    
    def precompute_amr_graphs(self):
        """预计算所有句子的AMR图"""
        sentences = [" ".join(item["sentences"][0]) for item in self.data]
        amr_graphs = parse_to_amr_batch(sentences)  # 批量处理
        return amr_graphs
    ...
    def __getitem__(self, idx):
        ...
        amr_graph = self.amr_graph_cache[idx]
        graph_data = amr_to_graph(amr_graph)
        
        # amr_vec, _ = get_graph_embeddings(graph_data) if graph_data else (torch.zeros(128), 1.0)
        if graph_data:
        ...
        ent1, ent2 = "", ""
        for (e1s, e1e, _), (e2s, e2e, _) in ent_pairs:
...
def main():
    ...
    test_dataset = RelationDataset(test_data)
