...
class RelationDataset(Dataset):
    ...
    def __init__(self, data, source="train", max_len=16, cache_dir="cache/amr_graphs/semeval_gpt"):
        self.data = data
        ...
        if os.path.exists(cache_path):
            try:
                with open(cache_path, "r", encoding="utf-8") as f:
                    content = f.read()
                    return safe_decode(content) if content.strip() else None
    ...
    def __getitem__(self, idx):
        ...
        if graph_data:
            ...
            graph_data = [Data(x=torch.zeros((1, 768)), edge_index=torch.tensor([[0],[0]]))]  # 占位图
        ...
        return combined, gold_relation_idx, graph_data[0]

def collate_fn(batch):
    features, labels, graphs = zip(*batch)
    features = torch.stack(features)
    labels = torch.tensor(labels, dtype=torch.long)
    return features, labels, list(graphs)
...
def get_graph_embeddings(graph_list, return_loss=False, temperature=0.5):
    global graph_model
    if graph_model is None:
    ...
    emb1 = F.normalize(graph_model(batch1.x, batch1.edge_index, batch1.batch), dim=1)  # [N, D]
    emb2 = F.normalize(graph_model(batch2.x, batch2.edge_index, batch2.batch), dim=1)  # [N, D]

    avg_emb = (emb1 + emb2) / 2  # 主任务输入使用

    if not return_loss:
        return avg_emb.cpu(), None

    # -------- NT-Xent 对比损失计算 --------
    N = emb1.shape[0]
    z = torch.cat([emb1, emb2], dim=0)  # [2N, D]

    # 计算 [2N, 2N] 的 cosine similarity 矩阵
    sim_matrix = torch.matmul(z, z.T) / temperature  # dot product 因为已经 normalize
    # 去除自身对比（对角线），避免影响 softmax
    mask = torch.eye(2 * N, dtype=torch.bool).to(device)
    sim_matrix = sim_matrix.masked_fill(mask, -1e9)

    # 构造 ground-truth 正样本索引（z[i] ↔ z[i+N] or z[i-N]）
    pos_indices = torch.arange(N, device=device)
    labels = torch.cat([pos_indices + N, pos_indices], dim=0)  # [2N]

    # 对每一行 softmax，然后计算 CE
    loss = F.cross_entropy(sim_matrix, labels)

    return avg_emb.cpu(), loss
...
def parse_to_amr(sentence):
    ...
    try:
        ...
        return safe_decode(graphs[0]) if graphs else None
    except:
...
def parse_to_amr_batch(sentences):
    ...
    try:
        ...
        return [safe_decode(g) if g else None for g in graphs]
    except:
...
# ---------- 训练与验证函数 ----------
def train(model, train_loader, optimizer, criterion, alpha=0.1):
    print("进入train函数")
    model.train()
    total_loss = 0
    correct = 0
    ...
    for features, labels, graphs in tqdm(train_loader, desc="Training", ncols=100, dynamic_ncols=True, leave=True):
        optimizer.zero_grad()
        ...
        # 图对比学习：计算图嵌入和对比损失
        graph_vec, contrastive_loss = get_graph_embeddings(graphs, return_loss=True)
        graph_vec = graph_vec.to(device)

        # 拼接图向量和实体对表示
        inputs = torch.cat([features[:, :768*2], graph_vec], dim=1)
...
def evaluate(model, val_loader, epoch=None):
    print(f"\n进入evaluate函数（Epoch {epoch}）")
    model.eval()
    graph_model.eval()
    ...
    for data in tqdm(val_loader, desc="Evaluating", dynamic_ncols=True, leave=True):
        inputs, labels, graphs = data
        ...
        if graphs:
            with torch.no_grad():
                emb, _ = get_graph_embeddings(graphs, return_loss=False)
            all_graph_embeddings.append(emb)
            all_graph_labels.extend(labels.cpu().tolist())
...
def main():
    # 加载数据
    with open("./dataset/semeval_gpt/train.json", "r", encoding="utf-8") as f:
        train_data = [json.loads(line) for line in f]
    with open("./dataset/semeval_gpt/test.json", "r", encoding="utf-8") as f:
    ...
    for epoch in range(epochs):
        ...
        val_acc = evaluate(model, test_loader, epoch=epoch+1)
